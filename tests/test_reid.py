"""ShotProfile / score / Background in isolation: exactness against the naive float model,
...
from framegate.reid import BITS, Background, ShotProfile, _unpack, bits, score
...
def test_scorer_matches_naive_float_model_exactly():
    for _ in range(500):
        ...
        p = profile(frames)
        q = int(rint())
        assert abs(score(bits(q), [p])[0] - naive_ll(q, p.counts, p.n)) < 1e-9
...
def test_counts_are_correct_and_finalize_is_idempotent():
    ...
    assert p.n == 50 and np.array_equal(p.logit, before)
...
def test_identical_frames_give_confident_bits():
    ...
    assert score(bits(h), [p])[0] > -3.2  # nats over 64 bits
    assert score(bits(h ^ ((1 << 64) - 1)), [p])[0] < -128.0
...
def test_within_shot_flicker_bits_do_not_penalise_a_rematch():
    ...
    same = score(bits(frame()), [p])[0]  # new frame, new mouth
    stable = [b for b in range(BITS) if b not in flicker]
    off = rng.choice(stable, 18, replace=False)
    diff = score(bits(base ^ sum(1 << int(b) for b in off)), [p])[0]  # different setup
...
def test_scorer_ranks_and_batches_consistently():
    ...
    q = bits(int(rint()))
    batched = score(q, profs)
    singly = [score(q, [p])[0] for p in profs]
...
def test_periodic_fold_matches_single_fold():
    ...
    assert np.allclose(incremental.logit, single.logit)
...
def test_fold_invalidates_cached_logs():
    ...
    p.finalize()
    assert p.n == 70
    assert np.allclose(p.logit, profile(np.concatenate([a, b])).logit)
...
def _accept_radius(h, n, eps, thresh=-17.92):
    ...
    for _ in range(n):
        p.add(h)
    p.finalize()
    ks = [k for k in range(BITS) if score(bits(h ^ ((1 << k) - 1)), [p])[0] >= thresh]
    return max(ks)
...
def test_empty_background_is_the_uniform_null():
    ...
    b = Background(0.02)
    assert abs(b.loglik(bits(int(rint()))) - BITS * np.log(0.5)) < 1e-9
...
def test_background_votes_once_per_prototype_not_once_per_frame():
    ...
    q = bits(h)
    llr = score(q, [long_p])[0] - b.loglik(q, 0)
...
def test_shared_bits_stop_supporting_a_match():
    ...
    profs = [
        profile(
            np.full(60, (common & ~mask) | int(rng.integers(0, mask + 1)), np.uint64)
        )
        for _ in range(8)
    ]
    b = _bg(list(enumerate(profs)))
    q = bits(profs[3].key_hash)
    ll = score(q, [profs[3]])[0]
...
def test_leave_one_group_out_matters_most_when_few_groups_are_known():
    ...
    for k in (2, 8):
        ...
        q = bits(profs[1].key_hash)
        ll = score(q, [profs[1]])[0]
...
def test_background_update_is_idempotent_per_prototype():
    ...
    b, q = _bg(list(enumerate(profs))), bits(int(rint()))
    before = b.loglik(q)
