...
@pytest.mark.parametrize("img, want_blank", [
...
@pytest.mark.parametrize("shape", [(8, 8, 3), (16, 16, 3), (1080, 200, 3),
                                   (200, 1080, 3), (1, 256, 3), (256, 1, 3)])
...
_VARIED = [
    synth.black(), synth.white(), synth.solid(128), synth.noise(seed=3),
    synth.gradient(), synth.checkerboard(cell=2), synth.stripes(period=2),
    synth.hsv_scene(60, 2), synth.grayscale_scene(4),
]
...
def test_map_and_scalar_ranges(img):
    fs = Gate().image(img)
    assert fs.saliency.shape == fs.fine_texture.shape == (G, G)
    assert fs.saliency.min() >= 0.0 and np.isfinite(fs.saliency).all()
    assert fs.fine_texture.min() >= 0.0 and np.isfinite(fs.fine_texture).all()
...
def test_temporal_signal_ranges_over_a_mixed_stream():
    ...
    frames = ([synth.noisy(scene) for _ in range(10)] +
...
def test_determinism_same_input_same_output():
    ...
    assert np.array_equal(a.fine_texture, b.fine_texture)
...
def test_fine_texture_responds_to_fine_not_smooth():
    fine = Gate().image(synth.checkerboard(cell=2)).fine_texture.max()
    smooth = Gate().image(synth.gradient()).fine_texture.max()
...
def test_saliency_localizes_a_distinct_patch():
    ...
    gy, gx = slice(int(40 / 128 * G), int(90 / 128 * G)), slice(int(40 / 128 * G), int(90 / 128 * G))
...
def test_global_brightness_shift_is_not_a_cut():
    ...
    g = Gate()
    cuts = []
    for f in [synth.noisy(scene) for _ in range(8)] + [synth.dim(scene, 0.6) for _ in range(8)]:
        _, sig = g.frame(f)
        cuts.append(sig.cut)
...
def test_steady_brightness_has_no_flicker_but_strobe_does():
    ...
    strobe = [s.flicker for s in _stream(
        [synth.noisy(synth.dim(scene, 1.0 if i % 2 == 0 else 0.4)) for i in range(40)])]
...
def test_noise_jitter_does_not_cause_cuts():
    scene = synth.hsv_scene(60, 2)
    assert not any(s.cut for s in _stream([synth.noisy(scene, amp=6) for _ in range(40)]))
...
def test_motion_present_under_movement_absent_when_static():
    ...
    assert float((fs.motion == 0).mean()) > 0.5              # a jittered-but-static scene is mostly zero
...
def test_motion_raw_when_floors_disabled():
    ...
    assert np.allclose(fs.motion, np.abs(fs.residual))       # both floors off -> raw magnitude
